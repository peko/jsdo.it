...
// デフォルトの武器
var WEPON_DEFAULT = {
    name: 'NORMAL BEAM', // 武器の名前
    power: 0.3, // 与えるダメージ 0~1
    speed: 3, // 弾のスピード
    length: 10, // 弾の長さ
    width: 1, // 弾の幅
    color: 'white', // 弾の色, 特殊武器の場合アイテムの色に反映される
    shootingInterval: 1000 * 0.35, // 連射速度
    // 貫通弾か示す, true の場合着弾しても消滅しない
    // explosion を指定した場合はそちらが優先される
    through: false,
    // 爆発による着弾後の範囲攻撃, 以下のプロパティを持つオブジェクトで指定する
    // 範囲攻撃の威力は武器の基本威力の半分
...
var UFO_INCIDENCE = 0.0035; // UFO の出現率 0~1
var SPECIAL_WEPON_TIME = 1000 * 20; // 特殊武器の持続時間
var SCORE = { // 各種スコア
    ASTEROID_DAMAGE: 10,
    ASTEROID_DESTROY: 50,
    UFO_DAMAGE: 0,
...
var isMouseDown = false; // マウスが押し込まれているか示す
var ship; // Ship
...
function init() {
    ...
    fieldRange = new Range(0, canvasWidth, 0, canvasHeight);
    ...
    for (var i = 0; i < 30; i++) {
        debris.push(new Debri(Akm2.randInt(canvasWidth)));
    }
...
function tweetLink() {
    ...
    dom.tweet.href = 'http://twitter.com/?status=GET ' + score + ' POINTS' + exc + ' http://jsdo.it/akm2/eVgJ - Asteroids';
...
Collection.prototype = Akm2.extend([], {
...
function Path(points, closed) {
    if (Akm2.isArray(points)) {
        for (var i = 0, len = points.length; i < len; i++) {
            this.push(points[i]);
        }
    }
    
    this.closed = Akm2.isUndefined(closed) ? true : closed;
}

Path.prototype = Akm2.extend([], {
    closed: true,
...
function Ship(x, y, size) {
    Akm2.Point.call(this, x, y);
    this.size = size; // 船のサイズ
    this.currentWepon = WEPON_DEFAULT;
    this.latest = this.clone(); // 前回の位置, 弾の射出時にのせるスピードを算出する
    this.path = new Path();
    this._referencePath = new Path(); // 基準点を示すパス, path の回転の基準とする
    this.v = new Akm2.Point(); // 移動ベクトル
    ...
    for (var i = 0, len = d.length; i < len; i++) {
        ...
        p = Akm2.Point.polar(r, c).add(this);
        this.path.push(p);
        this._referencePath.push(p.clone());
    }
}

Ship.prototype = Akm2.extend(new Akm2.Point(), {
    angle: 0, // 自機の向き
    ...
    // レーザーを射出, 射出が可能な場合は引数で渡された Collection に Beam を追加する
    fire: function(beams) {
        if (!this.possibleShooting) return false;
        this.possibleShooting = false;
        
        var p = Akm2.Point.polar(this.size, this.angle).add(this);
...
function Beam(x, y, angle, wepon) {
    ...
    Akm2.extend(this, wepon || WEPON_DEFAULT);
}

Beam.prototype = Akm2.extend(new Akm2.Point(), {
    ...
    update: function() {
        ...
        var v = Akm2.Point.polar(this.speed, this.angle);
        ...
        if (this.releaseCompleted) {
            ...
            // 射出が完了していないなら判定, 判定が完了なら長さを調整してフラグを立てる
            this.releaseCompleted = this.distanceTo(renderTail) > this.length;
            if (this.releaseCompleted) {
                renderTail.set(Akm2.Point.polar(this.length, this.angle - PI).add(this));
            }
        }
    },
...
function ExplosionPath(x, y, options) {
    this.x = x;
    this.y = y;
    Akm2.extend(this, options);
}

ExplosionPath.prototype = Akm2.extend(new Path(), {
    currentRange: 0,
    complete: false,
    ...
    update: function() {
        ...
        for (var i = 0, p; i < 10; i++) {
            p = Akm2.Point.polar(this.currentRange / 2, TWO_PI / 10 * i).add(this);
...
Asteroid.spawn = function() {
    var side = Akm2.randInt(3); // 出現位置をランダムに決定
    var angle = Akm2.randUniform(PI * 0.5);
    var x, y;
    
    // 0: Left, 1: Right
    if (side === 0 || side === 1) {
        y = Akm2.randUniform(canvasHeight + ASTEROID_MAX_SIZE, -ASTEROID_MAX_SIZE);
        x = (side === 0) ? - ASTEROID_MAX_SIZE : canvasWidth + ASTEROID_MAX_SIZE;
        angle = (side === 0) ? angle - PI * 0.25 : angle + PI * 0.75;
    }
    // 2: Top, 3: Bottom
    else {
        x = Akm2.randUniform(canvasWidth + ASTEROID_MAX_SIZE, -ASTEROID_MAX_SIZE);
    ...
    var radius = Akm2.randUniform(ASTEROID_MAX_SIZE, ASTEROID_MIN_SIZE);
    return new Asteroid(x, y, radius, angle);
};

Asteroid.prototype = Akm2.extend(new Akm2.Point(), {
    vanished: false,
    make: function() {
        this.v = Akm2.Point.polar(1, this.angle);
        this.v.normalize((1 - this.radius / ASTEROID_MAX_SIZE) * 1.75 + 0.25);

        this.path = new Path();
        for (var i = 0, num = 12, radius; i < num; i++) {
            radius = Akm2.randUniform(this.radius, this.radius * 0.5);
            this.path.push(Akm2.Point.polar(radius, TWO_PI * i / num).add(this));
        }
    },
    ...
    damage: function(damage, splinters) { 
        ...
        if (this.radius < ASTEROID_MIN_SIZE) {
            ...
            // 進行角度を最大 30 度ずらす
            this.angle += DEG_TO_RAD * Akm2.randUniform(30, -30);
            this.make();
        }
...
function Ufo(x, y) {
    ...
    var P = function(px, py) {
        return new Akm2.Point(x + px, y + py);
    };
    ...
    this.destination = new Akm2.Point(Akm2.randUniform(canvasWidth), Akm2.randUniform(canvasHeight));
    this.v = new Akm2.Point();
}

// ランダムなパラメータを与えられた新しい Ufo オブジェクトを返す
Ufo.spawn = function() {
    var x, y, side = Akm2.randInt(3); // 出現位置をランダムに決定
    
    // 0: Left, 1: Right
    if (side === 0 || side === 1) {
        y = Akm2.randUniform(canvasHeight + 30, -30);
        x = (side === 0) ? - 30 : canvasWidth + 30;
    }
    // 2: Top, 3: Bottom
    else {
        x = Akm2.randUniform(canvasWidth + 30, -30);
...
Ufo.prototype = Akm2.extend(new Akm2.Point(), {
    vanished: false,
    hp: 100, // UFO の HP
    ...
    update: function() {
        ...
        if (dist > UFO_SPEED) {
            ...
            destination.set(Akm2.randUniform(canvasWidth), Akm2.randUniform(canvasHeight));
        }
...
function Item(x, y) {
    ...
    var d = TWO_PI / 6;
    for (var i = 0; i < 6; i++) {
        path.push(Akm2.Point.polar(10, d * i).add(this));
    }
    
    this.wepon = WEPON_SPECIAL[Akm2.randInt(WEPON_SPECIAL.length - 1)];
    
    this.v = Akm2.Point.polar(ITEM_SPEED, Akm2.randUniform(TWO_PI));
}

Item.prototype = Akm2.extend(new Akm2.Point(), {
    update: function() {
        var v = this.v;
        this.add(v);
        this.path.eachPoints(function(p, i) { p.add(v); });
...
function Splinter(x, y, radius, num) {
    ...
    for (var i = 0; i < num; i++) {
        values.push({
            x: x, y: y,
            angle: Akm2.randUniform(TWO_PI),
            radius: Akm2.randUniform(radius),
            dist: Akm2.random()
        });
    }
...
Splinter.prototype = Akm2.extend(new Akm2.Point(), {
    vanished: false,
 ...
 */
function Debri(x) {
    Akm2.Point.call(this, x, Akm2.randInt(canvasHeight));
    this.speed = Akm2.randUniform(2, 0.5);
}

Debri.prototype = Akm2.extend(new Akm2.Point(), {
    vanished: false,
