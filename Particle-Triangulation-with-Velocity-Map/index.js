...
// Configs
var PARTICLE_NUM     = 100;　// 開始時パーティクル数
...
function createField(W, H) {
   ...
   for(var i=0; i<FIELD_BLUR_ITERATIONS; i++)  {
...
function loop() {
    ...
    ctx.fillRect(0, 0, w, h);
    if (DRAW_FIELD && field) ctx.putImageData(field, 0, 0);
    ...
    for (ilen = particles.length, i = 0; i < ilen; i++) {
        ...
        if (fp>0 && fp<fw*fh*4) {
            pa.vx -= (127 - field.data[fp  ])/FIELD_DECIPATION; // R chanel
            pa.vy -= (127 - field.data[fp+1])/FIELD_DECIPATION; // G chanel
        }
